...
$dtEmissaoIni21 = isset($_POST['dtEmissaoIni21']) ? $_POST['dtEmissaoIni21'] : null;
$dtEmisaoFim21  = isset($_POST['dtEmisaoFim21']) ? $_POST['dtEmisaoFim21'] : null;
$codCart21      = isset($_POST['codCart21']) ? $_POST['codCart21'] : null;

// Validar os dados
if (!$dtEmissaoIni21 || !$dtEmisaoFim21 ) {
    echo "Parâmetros obrigatórios ausentes.";
    exit;
}

// Converter datas para formato dd/mm/yyyy
$dtEmissaoIni21 = date("d/m/Y", strtotime($dtEmissaoIni21));
$dtEmisaoFim21  = date("d/m/Y", strtotime($dtEmisaoFim21));
...
$sql = "SELECT X.DT_EMISSAO_GUIA, X.AA_GUIA_ATENDIMENTO, X.NR_GUIA_ATENDIMENTO, X.NM_GRUPO, X.DT_VENC,
  ...
  FROM ( SELECT V.DT_EMISSAO_GUIA, V.CD_TIPO_GUIA, V.DS_TIPO_GUIA, V.AA_GUIA_ATENDIMENTO, V.NR_GUIA_ATENDIMENTO, V.NM_USUARIO, V.CD_UNIDADE_CARTEIRA, 
          ...
          WHERE V.DT_EMISSAO_GUIA BETWEEN to_date(:dataini,'dd/mm/yyyy') AND to_date(:datafim,'dd/mm/yyyy')
            AND V.CD_TIPO_GUIA = 21 /* EXAME - ATENDIMENTO  */
...
oci_bind_by_name($selectStmt, ':dataini', $dtEmissaoIni21);
oci_bind_by_name($selectStmt, ':datafim', $dtEmisaoFim21);
if($codCart21 <> null){
    oci_bind_by_name($selectStmt, ':carteira', $codCart21);
}
...
// $filename = "relatorio_guias_21_" . date("Ymd_His") . "_" . uniqid() . ".xlsx";
...
$filename = "relatorio_guias_21_" . date("Ymd_His") . ".csv";
