...
$dtEmissaoIni11 = isset($_POST['dtEmissaoIni11']) ? $_POST['dtEmissaoIni11'] : null;
$dtEmisaoFim11  = isset($_POST['dtEmisaoFim11']) ? $_POST['dtEmisaoFim11'] : null;
$codCart11      = isset($_POST['codCart11']) ? $_POST['codCart11'] : null;

// Validar os dados
if (!$dtEmissaoIni11 || !$dtEmisaoFim11 ) {
    echo "Parâmetros obrigatórios ausentes.";
    exit;
}

// Converter datas para formato dd/mm/yyyy
$dtEmissaoIni11 = date("d/m/Y", strtotime($dtEmissaoIni11));
$dtEmisaoFim11  = date("d/m/Y", strtotime($dtEmisaoFim11));
...
$sql = "SELECT X.DT_EMISSAO_GUIA, X.AA_GUIA_ATENDIMENTO, X.NR_GUIA_ATENDIMENTO, X.NM_GRUPO, X.DT_VENC,
  ...
  FROM ( SELECT V.DT_EMISSAO_GUIA, V.CD_TIPO_GUIA, V.DS_TIPO_GUIA, V.AA_GUIA_ATENDIMENTO, V.NR_GUIA_ATENDIMENTO, V.NM_USUARIO, V.CD_UNIDADE_CARTEIRA, 
           ...
           FROM GP.V_ANALITICO_GUIAS V 
           INNER JOIN GP.GUIA_HIS H ON H.AA_GUIA_ATENDIMENTO = V.AA_GUIA_ATENDIMENTO AND H.NR_GUIA_ATENDIMENTO = V.NR_GUIA_ATENDIMENTO
          ...
          WHERE V.DT_EMISSAO_GUIA BETWEEN to_date(:dataini,'dd/mm/yyyy') AND to_date(:datafim,'dd/mm/yyyy')
            AND V.CD_TIPO_GUIA = 11 /* CONSULTA - ATENDIMENTO */
 ...
 LEFT JOIN EMSFND.USUAR_MESTRE U1 ON H1.CD_USERID_ALT = U1.COD_USUARIO 
 LEFT JOIN EMSFND.USUAR_MESTRE U2 ON H2.CD_USERID_ALT = U2.COD_USUARIO ";
...
oci_bind_by_name($selectStmt, ':dataini', $dtEmissaoIni11);
oci_bind_by_name($selectStmt, ':datafim', $dtEmisaoFim11);
if($codCart11 <> null){
    oci_bind_by_name($selectStmt, ':carteira', $codCart11);
}
...
$filename = "relatorio_guias_11_" . date("Ymd_His") . ".csv";
